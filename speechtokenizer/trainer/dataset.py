...
class audioDataset(Dataset):
    ...
    def __init__(self, file_list, segment_size, sample_rate, downsample_rate=320, valid=False):
        super().__init__()
    ...
    def __getitem__(self, index):
        ...
        audio, sr = torchaudio.load(audio_file)  # audio dimensionality: (channel x time)
        feature = torch.from_numpy(np.load(feature_file))
        audio = audio.mean(axis=0)  # -> avergaed over channels -> dimensionality: (time)
        ...
        if audio.size(-1) > self.segment_size:
            ...
            feature_start = min(
                int(audio_start / self.downsample_rate), feature.size(0) - self.segment_size // self.downsample_rate
            )
