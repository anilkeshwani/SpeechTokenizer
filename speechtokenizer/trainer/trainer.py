...
import torch
from accelerate import Accelerator, DataLoaderConfiguration, DistributedDataParallelKwargs, DistributedType
...
class SpeechTokenizerTrainer(nn.Module):
    ...
    def __init__(
        ...
        (self.generator, self.optim_g, self.optim_d, self.scheduler_g, self.scheduler_d, self.dl, self.valid_dl) = (
    ...
    def save(self, path, best_dev_mel_loss):
        if best_dev_mel_loss < self.best_dev_mel_loss:
            ...
            torch.save(
                self.accelerator.get_state_dict(self.generator), f"{self.results_folder}/SpeechTokenizer_best_dev.pt"
            )
                    ...
                    with torch.inference_mode():
                        for i, batch in tqdm(enumerate(self.valid_dl)):
                            ...
                            if i < self.showpiece_num:
                                ...
                                self.log(
                                    {f"generate/x_hat_spec_{i}": plot_spectrogram(x_hat_spec[0].cpu().numpy())},
                                    type="figure",
                                    step=steps,
                                )
                        if not self.plot_gt_once:
